...
def _draw_overlay(frame, detections: list[dict], target: dict | None, reason: str):
    ...
    if target and target.get("target_type") == "multi_region":
        ...
        # draw every sub-region so the whole combo's gate is visible, not
        # just nothing (which is what happened before this branch existed:
        # target_type != "region" skipped all three shape checks below
        # silently, leaving a combo step's gate invisible on every frame).
